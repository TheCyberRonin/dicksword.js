...
class Client extends EventEmitter
{
  ...
  _handleWS(data)
  {
    ...
    case 'CHANNEL_CREATE':
      ...
      this.directMessages[wsData.recipients[0].id] = new Channel(wsData);
...
class Server extends Base
{
  constructor(data, client)
  {
    ...
    data.channels ? this.channels = new Channels(data.channels, client) : '';
  }
  /**
...
class Channels
{
  constructor(data, client)
  {
    ...
    for(let i = 0; i < len; i ++)
    {
      this[data[i].id] = new Channel(data[i]);
      client.channels[data[i].id] = new Channel(data[i]);
    }
  }
}
class Roles
